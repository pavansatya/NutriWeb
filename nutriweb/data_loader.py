...
# # Load the product dataset ensuring that 'code' is a string.
...
def get_product_by_code(barcode: str, df: pd.DataFrame) -> dict | None:
    code_str = str(barcode).strip()
    if code_str.isdigit() and len(code_str) < 13:
        code_str = code_str.zfill(13)

    result = df[df['code'] == code_str]
    if not result.empty:
        return result.iloc[0].to_dict()

    if code_str.startswith('0'):
        result = df[df['code'] == code_str.lstrip('0')]
        if not result.empty:
            return result.iloc[0].to_dict()

    return None

def get_index_by_code(barcode: str, df: pd.DataFrame) -> int | None:
    code_str = str(barcode).strip()
    result_idx = df.index[df['code'] == code_str]
    if len(result_idx) == 0:
        return None
    return result_idx[0]

def get_category_slice(product: dict, level: int, df: pd.DataFrame) -> pd.DataFrame:
    cat_col = f'category_level_{level}'
    ...
    if not cat_value:
        return df
    return df[df[cat_col] == cat_value]
